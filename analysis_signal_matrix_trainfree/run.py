...
SSM_CACHE = ROOT / "analysis_ssm/cache"
...
def extract_row_features(tag: str, rebuild: bool) -> tuple[dict[str, np.ndarray], dict]:
    ...
    cache = HERE / f"row_features_{tag}.npz"
...
def make_plots(
    ...
    fig.tight_layout()
    fig.savefig(HERE / "onset_aligned_four_signals.png", dpi=180)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(HERE / "signal_matrix.png", dpi=180)
    plt.close(fig)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(HERE / "fixed_t30_auc.png", dpi=180)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(HERE / "loop_precursor_vs_actions.png", dpi=180)
    plt.close(fig)
