...
# LEROBOT_HOME = Path(os.getenv("LEROBOT_HOME", "/datadrive1")).expanduser()
LEROBOT_HOME = Path(os.getenv("LEROBOT_HOME", "/mnt/blob")).expanduser()
...
class LeRobotDatasetMetadata:
    ...
    def create(
        ...
        if robot is not None:
            ...
            print("robot.robot_type:", robot.robot_type)
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def load_hf_dataset(self) -> datasets.Dataset:
        ...
        if self.episodes is None:
            ...
            parent = root_path.parent  # /mnt/data
            parent_cache = parent.parent / f"{parent.name}_cache"  # /mnt/data_cache
            cache_dir = str(parent_cache / root_path.name)  # /mnt/data_cache/OtherDataset
            file_list = sorted([str(file) for file in Path(path).rglob("*") if file.is_file()])
            
            # import json
            # with open("./data_files_new6.json", "w", encoding="utf-8") as f:
            #     json.dump(file_list, f, ensure_ascii=False, indent=4)
            hf_dataset = load_dataset("parquet", data_dir=path, data_files=file_list, split="train", cache_dir=cache_dir)
    ...
    def __getitem__(self, idx) -> dict:
        ...
        for attempt in range(max_retries):
            ...
            except (KeyError, IndexError, ValueError, TypeError, RuntimeError, OSError) as e:
                print(f"[Warning] Attempt {attempt+1}/{max_retries} failed at idx={idx} due to {e}")
                idx = random.randint(0, self.num_frames - 1)
        raise RuntimeError(f"Failed to load valid item after {max_retries} attempts.")

    def _get_item_internal(self, idx) -> dict:
...
class MultiLeRobotDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        root: list[str] | list[Path] | None = None,
        episodes: dict | None = None,
        image_transforms: Callable | None = None,
        delta_timestamps: list[dict[list[float]]] | None = None,
        tolerances_s: dict | None = None,
        download_videos: bool = True,
        local_files_only: list[bool] = None,
        video_backend: str | None = None,
    ):
        super().__init__()
        if data_weights is None:
            print("No data weight provided, using default weights of 1.0 for all datasets.")
            data_weights = [1.0] * len(repo_ids)
        ...
        self._datasets = [
            LeRobotDataset(
                ...
                delta_timestamps=delta_timestamps[i],
                tolerance_s=self.tolerances_s[repo_ids[i]],
                download_videos=download_videos,
                local_files_only=local_files_only[i],
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        if isinstance(index, int):
            ...
            assert 0 <= idx < len(self._datasets[dataset_id]), f"Index {idx} out of range for dataset {dataset_id} with length {len(self._datasets[dataset_id])}"
        elif isinstance(index, tuple):
            dataset_id, idx = index
            assert 0 <= idx < len(self._datasets[dataset_id]), f"Index {idx} out of range for dataset {dataset_id} with length {len(self._datasets[dataset_id])}"
        
        item = self._datasets[dataset_id][idx]
        item["dataset_index"] = torch.tensor(dataset_id)
        return item
