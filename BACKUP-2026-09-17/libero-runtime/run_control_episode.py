"""Closed-loop control experiment client: recurrence-triggered interventions against the HiMoE API servers.
...
W, THEILER = 14, 3
ARMS = ("native", "withdraw", "escalate", "resample_escape", "resample_random")
    ...
    try:
        ...
        while summary["action_steps"] < config.max_steps and not summary["success"]:
            ...
            if args.arm != "native" and detector.triggered() and cooldown == 0 and interventions_left > 0 and query >= W:
                ...
                n_done = MAX_INTERVENTIONS - interventions_left
                record = dict(query=query - 1, step=summary["action_steps"], rr=rr, index=n_done)
                if args.arm in ("withdraw", "escalate"):
                    operator = "withdraw4" if args.arm == "withdraw" else ("withdraw4", "side_plus", "side_minus")[n_done]
