"""[v4: adds hold16_random, a schedule-matched random-timed pause control] Closed-loop control experiment client: recurrence-triggered interventions against the HiMoE API servers.
...
W, THEILER = 14, 3
ARMS = ("native", "withdraw", "escalate", "resample_escape", "resample_random", "hold16", "withdraw8", "persist", "withdraw_early", "reprompt", "noise2", "retrace", "hold16_random")
TRIGGER_DIST = ROOT / "samples/moe-recurrence-20260916/first-trigger-distribution.json"
...
def main():
    ...
    random_first = None
    if args.arm == "hold16_random":
        dist = json.loads(TRIGGER_DIST.read_text())["first_trigger_queries_failures"]
        sched_rng = np.random.default_rng(np.random.SeedSequence([args.candidate_seed, 7, task_id, args.init_state_id, abs(hash(args.task_name or "")) % 1000003]))
        random_first = int(sched_rng.choice(dist))
    summary_extra = dict(random_first_query=random_first)
    ...
    summary = dict(status="failed", success=False, arm=args.arm, benchmark=args.benchmark, perturbation=args.perturbation,
                   ...
                   first_trigger_query=None, bitwise_matches=0, bitwise_checked=0, ports=ports, **summary_extra)
    ...
    try:
        ...
        boundary_positions = []
        last_gripper = -1.0
        interventions_left = MAX_INTERVENTIONS_V2 if args.arm in ("hold16", "withdraw8", "persist", "withdraw_early", "reprompt", "noise2", "retrace", "hold16_random") else MAX_INTERVENTIONS
        cooldown = 0
        resample_left = 0
        api_left = 0
        query = 0
        ...
        while summary["action_steps"] < config.max_steps and not summary["success"]:
            ...
            if cooldown > 0:
                cooldown -= 1
            # trigger logic
            fire = detector.triggered() if args.arm != "hold16_random" else (query - 1 >= random_first)
            if args.arm != "native" and fire and cooldown == 0 and interventions_left > 0 and query >= W:
                if summary["first_trigger_query"] is None:
                    summary["first_trigger_query"] = query - 1
                n_done = (MAX_INTERVENTIONS_V2 if args.arm in ("hold16", "withdraw8", "persist", "withdraw_early", "reprompt", "noise2", "retrace", "hold16_random") else MAX_INTERVENTIONS) - interventions_left
                ...
                if args.arm in ("reprompt", "noise2"):
                    ...
                    continue
                if args.arm in ("withdraw", "escalate", "hold16", "withdraw8", "persist", "withdraw_early", "retrace", "hold16_random"):
                    operator = {"withdraw": "withdraw4", "withdraw_early": "withdraw4", "hold16": "hold16", "hold16_random": "hold16", "withdraw8": "withdraw8", "retrace": "retrace"}.get(args.arm)
