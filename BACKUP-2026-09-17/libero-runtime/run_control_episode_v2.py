"""[v2: adds hold16 / withdraw8 / persist / withdraw_early arms] Closed-loop control experiment client: recurrence-triggered interventions against the HiMoE API servers.
...
W, THEILER = 14, 3
ARMS = ("native", "withdraw", "escalate", "resample_escape", "resample_random", "hold16", "withdraw8", "persist", "withdraw_early")
...
def main():
    ...
    try:
        ...
        boundary_positions = []
        last_gripper = -1.0
        interventions_left = MAX_INTERVENTIONS_V2 if args.arm in ("hold16", "withdraw8", "persist", "withdraw_early") else MAX_INTERVENTIONS
        cooldown = 0
        resample_left = 0
        query = 0
        mode = "policy"
        while summary["action_steps"] < config.max_steps and not summary["success"]:
            obs = build_policy_observation(observation, prompt)
            ...
            if args.arm != "native" and detector.triggered() and cooldown == 0 and interventions_left > 0 and query >= W:
                ...
                n_done = (MAX_INTERVENTIONS_V2 if args.arm in ("hold16", "withdraw8", "persist", "withdraw_early") else MAX_INTERVENTIONS) - interventions_left
                record = dict(query=query - 1, step=summary["action_steps"], rr=rr, index=n_done)
                if args.arm in ("withdraw", "escalate", "hold16", "withdraw8", "persist", "withdraw_early"):
                    operator = {"withdraw": "withdraw4", "withdraw_early": "withdraw4", "hold16": "hold16", "withdraw8": "withdraw8"}.get(args.arm)
                    ...
                    if operator in ("hold16", "grip_cycle"):
                        ...
                        steps_max = PHYS_STEPS_MAX * (2 if operator in ("withdraw8", "lift8") else 1)
