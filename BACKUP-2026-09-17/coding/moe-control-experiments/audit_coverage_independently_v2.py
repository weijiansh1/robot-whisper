"""Independent audit v2: signed-zero identity and measured logit arithmetic."""
...
def expected_inputs(parent, source, spec):
    ...
    if spec["generator"] == "noise":
        ...
        bias = np.copysign(bias, direction)
    return bias, noise
...
def main():
    ...
    for row in rows + references:
        ...
        with np.load(path, allow_pickle=False) as raw:
            ...
            if row["kind"] != "bare":
                ...
                # This is diagnostic only: FP32 softmax may follow BF16 logit addition.
    ...
    diagnostic_root = root / "logit-diagnostic"
    diagnostic = load(diagnostic_root / "result.json")
    diagnostic_protocol = load(diagnostic_root / "protocol.json")
    if not diagnostic["passed"] or not diagnostic["gpu_exact_logit_softmax"] or diagnostic["extra_model_calls_completed"] != 1:
        raise RuntimeError("Missing exact raw-logit diagnostic")
    diagnostic_script = Path(__file__).resolve().parent / "diagnose_coverage_logits.py"
    if sha(diagnostic_script) != diagnostic_protocol["script_sha256"]:
        raise RuntimeError("Diagnostic implementation changed")
    if sha(diagnostic_root / "response.npz") != diagnostic["raw_response_sha256"]:
        raise RuntimeError("Diagnostic raw response changed")
    reference = root / diagnostic["formal_reference_path"]
    if sha(reference) != diagnostic["formal_reference_sha256"]:
        raise RuntimeError("Diagnostic formal reference changed")
    softmax_error = 0.
    with np.load(diagnostic_root / "response.npz", allow_pickle=False) as raw, np.load(reference, allow_pickle=False) as original:
        for field in original.files:
            exact(raw[field], original[field], "raw-logit replay retained all original arrays")
        if len(diagnostic["logit_dtypes"]) != 1:
            raise RuntimeError("Mixed logit dtypes require separate analysis")
        dtype = {"torch.float32": torch.float32, "torch.bfloat16": torch.bfloat16}[diagnostic["logit_dtypes"][0]]
        logits = torch.tensor(raw["diagnostic/native_logits_fp32"], dtype=dtype)
        applied = torch.tensor(raw["gate_probe/request_bias"], dtype=dtype)
        exact(applied.float().numpy(), raw["diagnostic/applied_bias_fp32"], "actual logit bias cast")
        exact((logits + applied).float().numpy(), raw["diagnostic/effective_logits_fp32"], "actual rounded logit addition")
        for phase in ("native", "effective"):
            logits_fp32 = torch.tensor(raw["diagnostic/" + phase + "_logits_fp32"], dtype=torch.float32)
            error = float(np.max(np.abs(logits_fp32.softmax(-1).numpy() - raw["v8_control/" + phase + "_probs_fp32"])))
            softmax_error = max(softmax_error, error)
            if error > 3e-7:
                raise RuntimeError("CPU softmax of captured logits exceeds rounding allowance")
    result = dict(passed=True, raw_responses=len(rows), shared_responses=len(references), full_dispatch_responses=full_count,
                  ...
                  naive_fp32_logit_reweight_max_error=max_bias_probability_error,
                  naive_reweight_is_not_a_valid_check=True,
                  diagnostic_extra_model_forwards=diagnostic["extra_model_calls_completed"],
                  diagnostic_logit_dtypes=diagnostic["logit_dtypes"], diagnostic_probability_dtypes=diagnostic["probability_dtypes"],
                  diagnostic_rounded_logit_addition_exact=True, diagnostic_cpu_softmax_max_error=softmax_error,
                  diagnostic_cpu_softmax_atol=3e-7, diagnostic_result_sha256=sha(diagnostic_root / "result.json"),
                  audit_revision="v2: preserve signed zero and validate softmax using captured rounded logits",
