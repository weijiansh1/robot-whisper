"""Seal P3f, including the failed pilot, without changing older experiments."""
...
from run_crossover_experiment import check_config
...
def main(root):
    ...
    for name in ('REPORT.zh.md', 'EXECUTION_NOTES.zh.md', 'summary.json', 'crossover.png',
                 'scope-diagnostics/summary.json', 'cells.csv'):
    ...
    for name in ('p1a-20260914-my_4lkxd', 'p1b-gate-20260914-1nvvkzw2', 'p2a-coverage-20260914-4i18r353',
                 ...
                 'p3e-mechanism-20260915-9u_9j9b8'):
        old = BASE / 'runs' / name
    ...
    assert set(config['prior_pilot_files']) == {str(p.relative_to(pilot)) for p in pilot.rglob('*') if p.is_file()}
    ...
    result = dict(passed=True, source_hashes_unchanged=True, old_runs_unchanged=old_runs,
        prior_pilot_unchanged=True, shared_service_identity_unchanged=True,
        parameter_content_hash_unchanged=True, total_user_request_model_calls=142,
        post_collection_sources={str(BASE / name): digest(BASE / name) for name in
            ('analyze_crossover_experiment.py', 'plot_crossover_experiment.py', 'finalize_crossover_experiment.py')},
