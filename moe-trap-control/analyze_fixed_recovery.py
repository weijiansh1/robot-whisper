#!/usr/bin/env python3
"""Compare trigger policies with an unchanged recovery module on the same cohort."""
...
def summary(tasks):
    ...
    for reference in ("behavior_stall","random_time","v7_frozen","knn20","knn_euclidean_OR_cosine"):
        a=[outcome(t,"v8_frozen","withdraw") for t in tasks]
        b=[outcome(t,reference,"withdraw") for t in tasks]
        both=[(x,y) for x,y in zip(a,b) if x["triggered"] and y["triggered"]]
        comparisons[reference]=dict(full_cohort=paired(a,b),
    ...
    return dict(parents=len(tasks),original_failures=failures,original_successes=successes,
                methods=methods,v8_withdraw_comparisons=comparisons)
...
def run(args):
    ...
    report=dict(status="completed",audit=str(args.audit.resolve()),audit_sha256=digest(args.audit),
        analyzer_sha256=digest(__file__),stage="exploratory_fixed_module_trigger_comparison",
        estimand="net change in full-task success for a once-only trigger policy with fixed withdrawal, relative to native, across all selected parents",
        unit="one original main_id, one original policy-noise stream; shared events are not independent evidence",
        scope="fixed-hash category-stratified sample of 36 prior Long parents, including untriggered and original successes; not an official benchmark rate or held-out test",
        coverage="one maximum intervention per policy; actual trigger coverage differs and is reported, not claimed budget-matched",
        control="same fixed 16-step withdrawal for all policies, plus equal-duration hold; both consume original 520-step action budget",
    ...
    print(json.dumps(dict(parents=len(tasks),primary_parents=len(primary),
        methods={m:{a:{k:r[k] for k in ("successes","rescues","harms","effective_triggers")}
                    for a,r in arms.items() if a in ("hold","withdraw")} for m,arms in report["all"]["methods"].items()})))
