#!/usr/bin/env python3
"""Audit frozen MoE outcome choices and independently reconstruct all executed controllers."""
...
from control_bank import PHYSICAL, TRANSFORM
from moe_outcome_models import PROTOCOL, SETTINGS, ARMS, FAMILIES, MODELS, FeatureHistory, choose, derangement
...
def expected_operator(family, scores, tau, main_id, repeat, feature, task, plan):
    model_path = Path(plan["outcome_model_path"])
    require(digest(model_path) == plan["outcome_model_sha256"], "Frozen fitted model")
    fitted = json.loads(model_path.read_text())
    require(main_id not in fitted["training_main_ids"], "Training and online parent separation")
    if family in MODELS+("clock_knn",):
        return choose(fitted["model"], family, feature)[0]
    if family.startswith("shuffle_"):
        method = family[len("shuffle_"):]
        mapping = derangement([parent["main_id"] for parent in plan["tasks"]], method, repeat)
        donor = next(parent for parent in plan["tasks"] if parent["main_id"] == mapping[main_id])
        selected = choose(fitted["model"], method, donor["entry_feature"])[0]
        require(task["shuffled_choices"][family+"_r"+str(repeat)] ==
            dict(donor_main_id=donor["main_id"], operator=selected), "Independent matched shuffle")
        return selected
...
def audit_task(run, task, plan):
    ...
    for row in main[:q0]:
        feature_history.update(row[PROBS_KEY], int(row["action_steps_before"]))
    ...
    for arm in ARMS:
        ...
        feature = feature_history.probe(probe[PROBS_KEY], steps0)
        selection = json.loads((directory / "selection.json").read_text())
        require(result["selection_sha256"] == digest(directory / "selection.json"), "Committed actual selection")
        require(result["outcome_model_sha256"] == plan["outcome_model_sha256"] == selection["model_sha256"], "Fitted model identity")
        require(selection["actual_probe"] and selection["query"] == q0 and selection["action_steps_before"] == steps0, "Selection timeline")
        require(selection["arm"] == arm and selection["input_sha256"] == text(probe["input_sha256"]), "Selection input identity")
        np.testing.assert_allclose(feature, selection["feature"], rtol=0, atol=1e-8)
        np.testing.assert_allclose(feature, task["entry_feature"], rtol=0, atol=1e-8)
        for method in MODELS+("clock_knn",):
            predicted, utilities = choose(fitted["model"], method, feature)
            require(predicted == selection["models"][method]["operator"] == task["selector_choices"][method]["operator"], "Fitted choice recomputed")
            np.testing.assert_allclose(utilities, selection["models"][method]["scores"], rtol=0, atol=1e-6)
            np.testing.assert_allclose(utilities, task["selector_choices"][method]["scores"], rtol=0, atol=1e-6)
        operator = expected_operator(family, probe["component_scores"], threshold(q0,plan), main_id, repeat, feature, task, plan)
        require(selection["operator"] == operator, "Selected controller commitment")
    ...
    for repeat in (0,1):
        groups = {}
        for arm, (operator, physical, rows) in traces.items():
            if arm.endswith("_r"+str(repeat)):
                groups.setdefault(operator, []).append((physical, rows))
        for operator, repeated in groups.items():
            reference_physical, reference_rows = repeated[0]
            for physical, rows in repeated[1:]:
                equal_records(physical, reference_physical, "Same-operator physical dispatch")
                equal_records(rows, reference_rows, "Same-operator complete policy dispatch")
...
def audit_fresh_mains(plan):
    ...
    for task in (t for t in plan["cohort"] if t["cohort_group"] == "fresh"):
        ...
        for row in main:
            route_check(row)
        count += len(main)
    return dict(mains=len(plan["cohort"]),main_queries=count,c0_queries=count,all_exact=True)
...
def audit(args):
    ...
    fitted = json.loads(Path(plan["outcome_model_path"]).read_text())
    require(not set(fitted["training_main_ids"]) & {t["main_id"] for t in plan["cohort"]}, "Held-out main IDs")
    require({t["init_index"] for t in plan["cohort"]} == set(range(15,25)), "Held-out initialization indices")
    for method in MODELS:
        for repeat in (0,1):
            assigned = Counter(t["selector_choices"][method]["operator"] for t in plan["tasks"])
            shuffled = Counter(t["shuffled_choices"]["shuffle_"+method+"_r"+str(repeat)]["operator"] for t in plan["tasks"])
            require(assigned == shuffled, "Matched operator counts")
