#!/usr/bin/env python3
"""Audit actual v8.2-triggered branches and both fixed and moving score thresholds."""
...
from v8_closed_loop import PROTOCOL as NOISE_PROTOCOL
from v82_closed_loop import PROTOCOL, ARMS, SETTINGS, V82TriggerMonitor as TriggerMonitor
...
def check_pool(path, arm, row, live, native_noise, task, index, thresholds, margins):
    ...
    np.testing.assert_array_equal(selection["thresholds"], thresholds)
    np.testing.assert_array_equal(selection["margins"], margins)
    require(selection["selector_version"] == ("v82" if arm.endswith("_v82") else "v8"), "Wrong selector version")
    ...
    generator = np.random.default_rng(int(stable_id(NOISE_PROTOCOL, task["main_id"], index, "extra_noise")[:8], 16))
    ...
    if "_random_" in arm:
        rng = np.random.default_rng(int(stable_id(NOISE_PROTOCOL, task["main_id"], index, "selection")[:8], 16))
...
def audit_parent(payload):
    ...
    base_thresholds, margins = np.asarray(thresholds), np.asarray(margins)
    ...
    require(replay["c0"]["status"] == "passed" and replay["online_triggers_exact"] and replay["v82_triggers_exact"], "Exact C0 missing")
    ...
    require(live.first == task["first_alarms"] and prefix.first["v82_frozen"] == q0-1, "Frozen v8.2 timing changed")
    ...
    for arm in ARMS:
        ...
        for index, row in enumerate(suffix):
            ...
            thresholds = base_thresholds.copy()
            if arm.endswith("_v82"):
                thresholds[3:] -= .0015*int(row["query"])
            np.testing.assert_array_equal(row["selector_thresholds"], thresholds)
            ...
            require(monitor.v8.v7.query == int(row["query"]), "Candidate previews advanced history")
            require(bool(row["v82_alarm"]) == status["v82_alarm"] and int(row["v82_first"]) == status["v82_first"], "v8.2 latch differs")
    ...
    if first_pools:
        ...
        for family in ("iid", "guided"):
            for version in ("v8", "v82"):
                left, right = family+"_random_"+version, family+"_"+version
                if left in first_pools and right in first_pools:
                    for a, b in zip(first_pools[left], first_pools[right]):
                        for key in (*ALL_FIELDS, "actions", "noise", "input_sha256"):
    ...
    return dict(main_id=task["main_id"], benchmark=task["benchmark"], category=task["category"],
        ...
        trigger_heads=dict(freeze=prefix.v8.v7.first_freeze_query == q0-1,
            ...
            inversion=prefix.v8.v82_first[0] == q0-1, curvature=prefix.v8.v82_first[1] == q0-1),
...
def run(args):
    ...
    result["cohort"] = [dict(main_id=t["main_id"], base_task=t["base_task"], native_success=t["native_success"],
        native_queries=t["parent_queries"], first_v8_alarm=t["first_alarms"]["v8_frozen"],
        first_v82_alarm=t["first_alarms"]["v82_frozen"], effective_alarm=bool(t["events"])) for t in plan["cohort"]]
    result["reused_main_coverage"] = len(result["cohort"])
    result["verification"] += "; v8.2 online trigger, absolute-query thresholds, version-matched random controls"
