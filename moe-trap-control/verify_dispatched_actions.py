#!/usr/bin/env python3
"""Re-execute recorded action suffixes in MuJoCo without calling the policy."""
...
def verify(args):
    ...
    for arm in ("native", "iid_v8", "guided_v8"):
    ...
    result = dict(status="passed", run=str(args.run), main_id=args.main_id, benchmark=task["benchmark"],
        verification="Independent MuJoCo re-execution of every recorded action in three complete suffixes; no policy calls",
