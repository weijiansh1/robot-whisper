...
def stack_tree(rows):
    ...
    return torch.stack([torch.as_tensor(np.asarray(row) if isinstance(row, np.generic) else row)
                        for row in rows])
...
def run(args):
    ...
    try:
        ...
        companion_seeds = list(range(97100, 97108))
        companions = [probe.observation("goal", seed, capture=True) for seed in companion_seeds]
        for i, request in enumerate(requests + companions):
            request["prompt"] = prompts[i % len(prompts)]
        report["input_seeds"] = seeds
        report["input_prompts"] = [r["prompt"] for r in requests]
        report["companion_seeds"] = companion_seeds
        report["companion_prompts"] = [r["prompt"] for r in companions]
        ...
        configs = ([] if args.checks_only else [(True, 1)]) + [(False, b) for b in args.batches]
        for original, batch in configs:
            ...
            if not original:
                ...
                changed_companion = [requests[0]] + companions[:batch - 1]
