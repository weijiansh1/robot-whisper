...
def run(args):
    ...
    report = dict(run=str(root), audit_sha256=digest(args.audit), formal_collection=audit["formal_collection"],
        ...
        storage_projection=dict(disk_free_bytes=free_bytes, reserve_bytes=8 * 1024**3,
            ...
            limitation="Estimate for this Long batch's alarm frequency and suffix lengths, not a guaranteed maximum or all-suite estimate"))
    ...
    if audit["formal_collection"]:
        ...
        with index_path.with_suffix(".lock").open("a") as lock:
            ...
            combined["completed_count"] = len(combined["completed"])
            combined["screen_planned_remaining"] = 3800 - combined["completed_count"]
            combined["coverage_planned_remaining"] = 14030 - combined["completed_count"]
