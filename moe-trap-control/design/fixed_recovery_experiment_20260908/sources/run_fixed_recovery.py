...
from fixed_recovery_control import PROTOCOL, MODULE, ARMS, METHODS
...
def run(args):
    ...
    if (plan["protocol"]!=PROTOCOL or plan["module"]!=MODULE or plan["arms"]!=list(ARMS) or
            plan["methods"]!=list(METHODS) or plan["allowed_gpus"]!=[0,1,2,3]):
    ...
    source_names=("fixed_recovery_control.py","collect_fixed_recovery.py","run_fixed_recovery.py",
        ...
        try:
            while not stop.is_set():
                ...
                if benchmark not in workers:
                    ...
                    command=[str(BASE/"envs/libero/bin/python"),"-u",str(HERE/"collect_fixed_recovery.py"),
                ...
                with lock:
                    ...
                    if passed and job["kind"]=="fixed_replay":
                        for child in children[task["main_id"]]:
                            ...
                            ready.put((-3*(52-jobs[child]["event"]["start_query"]),child))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port-base",type=int,default=11600)
