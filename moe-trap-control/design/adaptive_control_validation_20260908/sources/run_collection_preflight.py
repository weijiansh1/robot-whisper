...
def run(args):
    ...
    reuse = continuation or control
    plan_path = args.control_plan if control else args.continuation_plan if continuation else args.plan
    ...
    report = dict(status="loading", started_utc=probe.now(), gpus=args.gpus, excluded_gpu=6,
        ...
        continuation_experiment=continuation, adaptive_control_experiment=control, new_main_coverage=not reuse,
        scheduling="c0_then_independent_event_repeat_queue" if control else "largest_reserved_work_first" if continuation else "frozen_queue_order",
    ...
    def env_worker(gpu, worker_index):
        ...
        try:
            while not stop.is_set() and not dispatch_stop.is_set():
                try:
                    index = pending.get(timeout=.5) if control else pending.get_nowait()
                except queue.Empty:
                    if control:
                        with lock:
                ...
                if benchmark not in workers:
                    worker_script = "collect_adaptive_control.py" if control else "collect_long_continuation.py" if continuation else "collect_preflight_worker.py"
                ...
                with lock:
                    ...
                    if control and status == "completed":
