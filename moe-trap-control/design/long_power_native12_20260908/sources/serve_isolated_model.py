...
def child_replica(args, endpoints, policies, uuid, replica, owner_pid):
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", force=True)
    try:
        result = serve_replica(args, endpoints, policies, uuid, replica, owner_pid)
    finally:
        policies.clear()
        gc.collect()
        import torch
...
def main():
    ...
    parser.add_argument("--replicas", type=int, choices=(1, 2, 4, 8, 12, 16), default=1)
    ...
    try:
        if args.replicas > 1:
            ...
            for replica in range(1, args.replicas):
                child_endpoints = tuple(bundle.ModelEndpoint(e.name, e.kind, e.port + 5 * replica) for e in endpoints)
                child = torch.multiprocessing.get_context("spawn").Process(target=child_replica,
                    args=(args, child_endpoints, policies, uuid, replica, os.getpid()), name="cuda-ipc-replica")
                child.start()
        ...
        stopping.set()
        # Keep the CUDA storage owner alive until all IPC consumers have exited.
        for child in children:
            if child.is_alive():
                child.terminate()
