...
class FullASCapture:
    ...
    def response(self):
        ...
        probability, ids, weights = stack(1, torch.float16), stack(2, torch.int16), stack(3, torch.float32)
        ...
        if not np.array_equal(selected, weights) or not np.array_equal(selected[..., 0], probability.max(-1)):
            raise RuntimeError("AS probabilities differ from actual top-1 weights")
        return dict(zip(AS_FIELDS, (probability, ids, weights, ids.copy(), weights.copy())))
