...
from collection_protocol import PARAMETERS_SHA256, load_plan
...
def run(args):
    ...
    if digest(HERE / "design/frozen_alarm_comparison_20260908/profiles/parameters.json") != PARAMETERS_SHA256:
    ...
    if args.plan is not None:
        ...
        plan = load_plan(args.plan, args.model)
        inventory = {row["variant_id"]: row for benchmark in ("pro", "plus") for row in variants(benchmark)}
    ...
    sources = [HERE / name for name in ("run_collection_preflight.py", "collect_preflight_worker.py",
               ...
               "audit_collection_preflight.py", "audit_collection_experiment.py")]
    ...
    report = dict(status="loading", started_utc=probe.now(), gpus=args.gpus, excluded_gpu=6,
        ...
        tasks=[dict(variant=row, main_id=row.get("main_id"), status="queued",
                    output=str(args.output / "tasks" / row.get("main_id", row["variant_id"]))) for row in jobs])
    if plan is not None:
                ...
                for task in report["tasks"]:
                    ...
                    if task["status"] == "running" and path.exists():
                        ...
                        task.update(queries=progress["queries"], action_steps=progress["action_steps"],
                                    branches=progress.get("branches", []), main_complete=progress["main_complete"])
