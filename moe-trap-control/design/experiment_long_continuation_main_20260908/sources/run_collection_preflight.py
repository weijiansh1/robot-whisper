...
def run(args):
    ...
    plan_path = args.continuation_plan if continuation else args.plan
    ...
    report = dict(status="loading", started_utc=probe.now(), gpus=args.gpus, excluded_gpu=6,
        ...
        continuation_experiment=continuation, new_main_coverage=not continuation,
        scheduling="largest_reserved_work_first" if continuation else "frozen_queue_order",
        ...
        try:
            while not stop.is_set() and not dispatch_stop.is_set():
                ...
                if benchmark not in workers:
                    worker_script = "collect_long_continuation.py" if continuation else "collect_preflight_worker.py"
