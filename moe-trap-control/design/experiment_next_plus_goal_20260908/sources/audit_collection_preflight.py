...
def audit_task(directory, profile, paired=False):
    ...
    for row in records(directory / "main"):
        ...
        if "proprio" in row:
            require(np.isfinite(row["proprio"]).all() and hashlib.sha256(row["proprio"].tobytes()).hexdigest() ==
                    row["input_component_sha256"][2].decode(), "Actual proprio input hash")
        for field in FIELDS:
            value = row[field]
    ...
    for actual in records(directory / "c0"):
        ...
        if "proprio" in expected:
            np.testing.assert_array_equal(actual["proprio"], expected["proprio"])
        compared += 1
...
def audit_as(row):
    ...
    require(np.array_equal(selected, weights.astype(np.float16)) and np.array_equal(selected[..., 0], probs.max(-1)), "AS actual combine weight")
