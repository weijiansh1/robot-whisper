#!/usr/bin/env python3
"""Build compact graph-dynamics profiles from the original router tensors."""
...
DEFAULT_OUTPUT = BUNDLE / "results/structural_profiles"
...
def half_with_error(values: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    if not finite.any():
        return half, 0.0
    error = float(np.max(np.abs(half.astype(np.float32)[finite] - values[finite])))
    return half, error
...
def build_one(
    ...
    final_views = {
        name: np.empty((count, 8, view_width[name]), dtype=np.float32) for name in VIEW_NAMES
    }
...
def main() -> None:
    ...
    processes = [
        context.Process(
            target=worker,
            args=(device, partitions[device], str(args.output), args.batch, args.compressed, args.resume, queue),
        )
    ...
    summary = {
        "schema": "himoe.graph_homeostasis_profile_build.v1",
        ...
        "storage": "deflate" if args.compressed else "zip-store",
