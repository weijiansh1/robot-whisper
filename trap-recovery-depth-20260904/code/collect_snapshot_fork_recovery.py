#!/usr/bin/env python3
"""How far into a trap can it still be pulled out?

Same collector as himoe-vla_trap/code, with one constraint relaxed: fork
offsets may now be positive, i.e. AFTER the physical loop onset rather than
only before it.  Everything else -- the frozen onset rule, the paired noise
streams, the untouched-trunk control -- is unchanged.
...
def parse_queries(value: str) -> Tuple[int, ...]:
    queries = tuple(int(part.strip()) for part in value.split(",") if part.strip())
    if not queries or len(set(queries)) != len(queries) or any(q < 0 for q in queries):
        raise argparse.ArgumentTypeError("fork queries must be distinct non-negative ints")
    return queries
...
def parse_offsets(value: str) -> Tuple[int, ...]:
    ...
    if not offsets or len(set(offsets)) != len(offsets):
        raise argparse.ArgumentTypeError("offsets must be distinct comma-separated integers")
    return offsets
...
def main() -> int:
    ...
    parser.add_argument("--init-state-id", type=int, default=3)
    ...
    parser.add_argument(
        ...
        default=str(PACKAGE_ROOT / "configs/task08_physical_trap_definitions.json"),
    )
    parser.add_argument(
        "--fork-queries", type=parse_queries, default=None,
        help="absolute query indices to fork at; overrides --fork-offsets. "
             "Depth into the trap becomes fork_query - onset, so every trunk "
             "contributes and residual budget is held fixed across trunks.")
    ...
    try:
        ...
        with PolicyClient(
            ...
            if trunk_success or (onset < 0 and not args.fork_queries):
            ...
            if args.fork_queries:
                fork_points = [(q, q - onset if onset >= 0 else None, "q%02d" % q)
                               for q in args.fork_queries]
            else:
                fork_points = [(onset + o, o, "fork_%+d" % o)
                               for o in args.fork_offsets]
            for offset_index, (fork_query, depth, label) in enumerate(fork_points):
                ...
                candidate_summaries = []
                print("fork %s q=%d depth=%s budget=%d" % (label, fork_query, depth,
                      args.max_trunk_queries - fork_query), flush=True)
