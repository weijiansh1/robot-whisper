...
const html = readFileSync(
  new URL("../viz/himoe-moe-case-spread.html", import.meta.url), "utf8");
const script = html.slice(html.lastIndexOf("<script>") + 8, html.lastIndexOf("</script>"));
new Function(script)();

// ---- checks -------------------------------------------------------------
let fail = 0, warn = 0;
const CHART_IDS = ["c1","c2","c3"];
...
for (const t of ["tbl", "tb4", "tiles", "lg1"]) {
  const host = ids[t];
  const ok = /<table|<div|<span/.test(host.innerHTML || "");
  console.log(`${ok ? "[PASS]" : "[FAIL]"} ${t}: ${ok ? "rendered" : "EMPTY"}`);
  if (!ok) fail++;
}
