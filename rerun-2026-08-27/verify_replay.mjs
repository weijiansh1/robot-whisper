...
const html = readFileSync(new URL("../viz/himoe-moe-replay.html", import.meta.url), "utf8");
...
function walk(tag) {
  for (let v = 0; v <= 1000; v += 3) {
    ...
    catch (e) { console.log(`[FAIL] ${tag} threw at ${v}: ${e.message}`); errors++; continue; }
    steps++;
    const t = parseFloat(clk.textContent)/12;
    let sc = 0;
    for (let k = 0; k < BOUND.length; k++) if (t >= BOUND[k]) sc = k;
    sceneOps[sc] = Math.max(sceneOps[sc] || 0, ops.length);
    if (ops.length < 6) empty++;
    if (bad.length) {
      console.log(`[FAIL] ${tag} ${v}: ${[...new Set(bad)].slice(0, 2).join(" | ")}`);
      errors += bad.length;
    }
    for (let i = 0; i < texts.length; i++)
      for (let j = i + 1; j < texts.length; j++) {
        ...
        if (a.x0 < b.x1 - 2 && b.x0 < a.x1 - 2 && a.y0 < b.y1 - 2 && b.y0 < a.y1 - 2) {
          console.log(`[WARN] ${tag} scrub=${v}: "${a.s}" x "${b.s}"`);
          collisions++;
        }
      }
  }
...
walk("plain");
/* The side-by-side compare overlay draws an extra label and an extra trail that
   the plain walk never exercises; toggle it on and walk the timeline again. */
const stub = { target: { classList: { toggle() {}, add() {}, remove() {} } } };
const cmpOps = () => Object.values(sceneOps).reduce((a, b) => a + b, 0);
const before = cmpOps();
get("cmp").fire("click", stub);
walk("compare");
if (cmpOps() <= before) { console.log("[FAIL] compare toggle drew nothing"); errors++; }
get("cmp").fire("click", stub);   // leave it off again
...
const allScenes = [...Array(5).keys()].every(s => sceneOps[s]);
