...
const W = 1280, H = 720;
let ops = [], bad = [], texts = [];
var shapes = {verts:0,dots:0,bars:0};
var probe = {};
const track = (k, x, y) => {
...
function makeCtx() {
  return {
    ...
    moveTo(x, y) { track("moveTo", x, y); shapes.verts++; },
    lineTo(x, y) { track("lineTo", x, y); shapes.verts++; },
    rect(x, y) { track("rect", x, y); },
    roundRect(x, y, w, h) {
      track("roundRect", x, y);
      if (!(w >= 0) || !(h >= 0)) bad.push(`roundRect ${w}x${h}`);
      if (w > 60 && h > 10) shapes.bars++;
      ops.push("r");
    },
    arc(x, y, r) { track("arc", x, y); if(!(r>=0)) bad.push(`arc r=${r}`); if(r>2) shapes.dots++; ops.push("a"); },
...
const html = readFileSync(new URL("../viz/himoe-moe-reversal.html", import.meta.url), "utf8");
...
const scrub = get("s"), clk = get("t");
const BOUND = [0, 7.5, 15.5, 26.5, 38.5, 47, 56, 66];
let errors = 0, collisions = 0, steps = 0, empty = 0;
const sceneOps = {};
for (let v = 0; v <= 1000; v += 3) {
  ops = []; bad = []; texts = []; shapes = {verts:0,dots:0,bars:0};
  ...
  { let s2=0; for(let k=0;k<BOUND.length;k++) if(t>=BOUND[k]) s2=k;
    const pp=(t-BOUND[s2])/((BOUND[s2+1]??70)-BOUND[s2]);
    if(pp>0.72 && pp<0.96) probe[s2]={...shapes, texts:texts.length}; }
  if (bad.length) {
...
for (let s = 0; s < 8; s++)
  console.log(`  ${sceneOps[s] ? "[PASS]" : "[FAIL]"} scene ${s + 1}: peak ${sceneOps[s] || 0} ops`);
const allScenes = [...Array(8).keys()].every(s => sceneOps[s]);
const ok = !errors && !collisions && allScenes;
const want=[["dots",20],["verts",40],["verts",60],["verts",30],["bars",2],["bars",2],["bars",3],["verts",6]];
let cfail=0;
console.log("\ncontent present at each scene's 72-96% point:");
want.forEach(([key,need],i)=>{const p=probe[i]||{};const got=p[key]||0;
 const okc=got>=need; if(!okc)cfail++;
 console.log(`  ${okc?"[PASS]":"[FAIL]"} scene ${i+1}: ${key}=${got} (need >=${need}), labels=${p.texts||0}`);});
console.log("\n"+((ok&&!cfail)?"REVERSAL OK":"REVERSAL HAS FAILURES"));
process.exit(ok ? 0 : 1);
