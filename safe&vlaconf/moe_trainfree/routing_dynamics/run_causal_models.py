"""Retrospective absolute-scale and sequential routing model experiment."""
...
from causal_models import COMPONENTS, DECAY, DRIFT, METHODS, PRIMARY, SCHEMA, make_components, model_scores, reference_evidence
...
PREVIOUS = BASE / "state_action_combinations_20260909"


def inputs():
    frame, features, valid, hashes = verified_inputs()
    for directory, names in ((BASELINE, ("relations.npz",)), (PREVIOUS, ("predictions.npz",))):
        manifest = json.loads((directory / "final_verification.json").read_text())
        for name in names:
            path = directory / name
            assert digest(path) == manifest["artifacts"][name]
            hashes[str(path)] = digest(path)
    with np.load(BASELINE / "relations.npz", allow_pickle=False) as z:
        np.testing.assert_array_equal(z["global_rows"], frame.global_row)
        np.testing.assert_array_equal(z["valid"], valid)
        branches = dict(**dynamics_scores(features, valid), **confirmed_relations(z["features"], valid))
    return frame, valid, make_components(branches, features, valid), hashes
...
def calibrate(frame, valid, components, out):
    ...
    for fold, ref, cal, test in state_splits(frame):
        ...
        refs = {name: PeakBank(np.where(np.isfinite(value[ref_success]), value[ref_success], -np.inf).max(1))
        ...
        profile = dict(schema=SCHEMA, fold=fold, methods=list(METHODS), primary=PRIMARY,
                       decay=DECAY, drift=DRIFT, base_profile=os.path.relpath(base_path, out),
                       base_profile_sha256=digest(base_path), references={name: bank.to_list() for name, bank in refs.items()},
                       banks=banks, reference_rows=ref, reference_success_rows=ref_success,
                       calibration_rows=cal, calibration_success_rows=cal[success], test_rows=test,
        ...
        print(f"CALIBRATED causal models fold={fold}", flush=True)
...
def predict(frame, valid, components, profiles, out):
    ...
    with np.load(PREVIOUS / "predictions.npz", allow_pickle=False) as z:
        ...
        old_index = list(z["methods"]).index("ac_recalibrated")
        ...
        controls = np.stack((z["control_first"][:, :, list(z["control_names"]).index("old_ac")],
                             z["first"][:, :, list(z["methods"]).index("two_of_three")]), axis=2)
    ...
    saved = dict(scores=scores, tails=tails, first=first, frozen_first=frozen, control_first=controls,
                 control_names=np.asarray(("old_ac", "previous_two_of_three")), old_ac_rank=old_ac_rank,
                 raw_v82_scores=v82, global_rows=rows_b, valid=valid[rows_b], methods=np.asarray(METHODS),
    ...
    print("PREDICTED: AC reference matches previous scores, tails and all first alarms", flush=True)
    return b, saved
...
def main():
    ...
    names = ("causal_models.py", "run_causal_models.py", "model_tables.py", "CAUSAL_MODELS_PROTOCOL_ZH.md",
    ...
    write_json(out / "execution_contract.json", dict(retrospective=True, no_classifier_training=True,
        ...
        B_parameter_selection=False, inputs=hashes, sources={name: digest(HERE / name) for name in names}))
