...
from causal_models import COMPONENTS, METHODS, PRIMARY, CausalModelMonitor
from guard import ALPHAS, KINDS, PeakBank
from run_causal_models import BASE, HERE, inputs
...
def raw_replay(b, components, saved, profiles, output):
    ...
    errors, component_errors = dict.fromkeys(METHODS, 0.), dict.fromkeys(COMPONENTS, 0.)
    ...
    for number, ix in enumerate(selected):
        ...
        live = CausalModelMonitor(output / f"fold_{profile['fold']}_profile.json", row.checkpoint)
        ...
        if (number + 1) % 20 == 0:
            print(f"RAW CAUSAL {number+1}/{len(selected)} episodes, {queries} queries", flush=True)
