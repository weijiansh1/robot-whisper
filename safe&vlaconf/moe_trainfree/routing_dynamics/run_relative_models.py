...
from relative_models import COMPONENTS, METHODS, PRIMARY, SCHEMA, WINDOW, make_components, model_scores, reference_evidence
...
def inputs():
    ...
    for directory, names in ((BASELINE, ("relations.npz",)), (PREVIOUS, ("predictions.npz", "query_auroc.csv"))):
    ...
    with np.load(BASELINE / "relations.npz", allow_pickle=False) as z:
        ...
        components = make_components(features, z["features"], valid)
...
def main():
    ...
    names = ("relative_models.py", "run_relative_models.py", "model_tables.py", "RELATIVE_MODELS_PROTOCOL_ZH.md",
