...
from relative_direction_models import COMPONENTS, RAW_NAMES, METHODS, PRIMARY, RelativeDirectionMonitor
from guard import ALPHAS, KINDS, PeakBank
from run_relative_direction_models import BASE, HERE, inputs
...
def raw_replay(b, components, saved, profiles, output):
    ...
    for number, ix in enumerate(selected):
        ...
        live = RelativeDirectionMonitor(output / f"fold_{profile['fold']}_profile.json", row.checkpoint)
        ...
        if (number + 1) % 20 == 0:
            print(f"RAW RELATIVE DIRECTION {number+1}/{len(selected)} episodes, {queries} queries", flush=True)
